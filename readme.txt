...
**Extraction de Caractéristiques :** Utiliser un modèle VGG19 pré-entrainé sur la base de données ‘imagenet’ et d’explorer d’autres alternatives notamment RESNET et INCEPTION pour extraire les caractéristiques des images, et le LSTM pour extraire les caractéristiques pertinentes à partir des coefficients MFCC des enregistrements audio.
**Classification Précise :** Développer des modèles de machine learning et de réseaux de neurones denses pour les images et des LSTM pour l'audio afin d'atteindre une classification précise des différentes scènes.
**Analyse des Performances :** Évaluer les modèles via des matrices de confusion et des métriques comme la précision, le rappel et rapport de classification pour identifier les forces et les faiblesses.
**Intégration Multimodale :** Explorer l'impact de l'utilisation de différentes architectures et méthodes de fusion des données, notamment la méthode tardive ( late fusion), précoce ( early fusion) et hybride sur la performance de notre système final.
